...
lab_dir = "pilot_data/pilot 1b/uchicago"
source(here("metadata/pod.R"))

d <- readxl::read_xlsx(here(lab_dir, "raw_data/UChicagoMB21b.xlsx"))
p <- read_csv(here(lab_dir, "raw_data/uchicago_babylab_pilot 1b_participantsheet.csv"))

# datasets
# dataset_id, monitor_size_x, monitor_size_y, sample_rate, tracker, lab_dataset_id
datasets <- tibble(dataset_id = 8, 
                   monitor_size_x = 1920,
                   monitor_size_y = 1200,
                   sample_rate = 60, 
                   tracker = "tobii", 
